    ...
    <div className="bg-white rounded-2xl shadow-sm border border-primary/15 overflow-hidden hover:border-primary/30 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 mb-3">
          ...
          <span className="text-sm font-medium">{(partner?.rating ?? 0) > 0 ? (partner?.rating ?? 0).toFixed(1) : '-'}</span>
          <span className="text-xs text-tmuted">{(partner?.total_reviews ?? 0) > 0 ? `(${partner?.total_reviews} รีวิว)` : 'ยังไม่มีรีวิว'}</span>
        </div>
