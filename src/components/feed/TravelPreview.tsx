          ...
          {flightRoutes.map((route) => (
            <a key={route.to} href={buildFlightLink(route.from, route.to)} target="_blank" rel="noopener noreferrer" className="bg-white rounded-2xl overflow-hidden border border-primary-dark/20 hover:border-secondary hover:shadow-md transition-all group">
              <div className="relative h-36 overflow-hidden bg-gradient-to-br from-primary/30 to-secondary/30">
                ...
                <div className="absolute bottom-2 left-3"><h3 className="font-bold text-white text-sm drop-shadow">{route.name}</h3><p className="text-white/70 text-[11px] drop-shadow flex items-center gap-1">{route.from} <ArrowRight size={10} /> {route.to}</p></div>
              </div>
              <div className="p-3 flex items-center justify-between"><span className="text-xs text-tmuted">เปรียบเทียบราคา</span><span className="text-[10px] text-tmuted group-hover:text-secondary transition flex items-center gap-0.5">ดูเที่ยวบิน <ExternalLink size={10} /></span></div>
            </a>
          ))}
        </div>
      </div>
          ...
          {hotelDestinations.map((dest) => (
            <a key={dest.nameEn} href={buildHotelLink(dest.nameEn)} target="_blank" rel="noopener noreferrer" className="bg-white rounded-2xl overflow-hidden border border-primary-dark/20 hover:border-secondary hover:shadow-md transition-all group">
              <div className="relative h-36 overflow-hidden bg-gradient-to-br from-primary/30 to-secondary/30">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                <div className="absolute bottom-2 left-3"><h3 className="font-bold text-white text-sm drop-shadow">{dest.name}</h3><p className="text-white/70 text-[11px] drop-shadow">{dest.nameEn}</p></div>
              </div>
              <div className="p-3 flex items-center justify-between"><span className="text-xs text-tmuted">เปรียบเทียบราคา</span><span className="text-[10px] text-tmuted group-hover:text-secondary transition flex items-center gap-0.5">ดูโรงแรม <ExternalLink size={10} /></span></div>
            </a>
          ))}
        </div>
      </div>
