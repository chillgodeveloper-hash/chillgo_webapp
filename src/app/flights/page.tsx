...
const popularRoutes = [
  ...
  { from: 'BKK', to: 'HKT', name: 'ภูเก็ต', nameEn: 'Phuket', image: 'https://photo.hotellook.com/static/cities/960x720/HKT.jpg', desc: 'หาดทรายขาว ทะเลอันดามัน ป่าตอง' },
  ...
  { from: 'BKK', to: 'USM', name: 'สมุย', nameEn: 'Koh Samui', image: 'https://photo.hotellook.com/static/cities/960x720/USM.jpg', desc: 'เกาะสวรรค์ รีสอร์ทหรู หินตาหินยาย' },
  { from: 'BKK', to: 'HDY', name: 'หาดใหญ่', nameEn: 'Hat Yai', image: 'https://photo.hotellook.com/static/cities/960x720/HDY.jpg', desc: 'ตลาดกิมหยง ไก่ทอด ตลาดสดใหญ่' },
  ...
  { from: 'BKK', to: 'URT', name: 'สุราษฎร์ธานี', nameEn: 'Surat Thani', image: 'https://photo.hotellook.com/static/cities/960x720/URT.jpg', desc: 'ประตูสู่เกาะสมุย เขื่อนรัชชประภา' },
  { from: 'BKK', to: 'NST', name: 'นครศรีธรรมราช', nameEn: 'Nakhon Si Thammarat', image: 'https://photo.hotellook.com/static/cities/960x720/NST.jpg', desc: 'วัดพระมหาธาตุ ขนมจีน เขาหลวง' },
  { from: 'BKK', to: 'UBP', name: 'อุบลราชธานี', nameEn: 'Ubon Ratchathani', image: 'https://photo.hotellook.com/static/cities/960x720/UBP.jpg', desc: 'สามพันโบก ผาแต้ม แก่งหินผาสาด' },
  { from: 'BKK', to: 'UTH', name: 'อุดรธานี', nameEn: 'Udon Thani', image: 'https://photo.hotellook.com/static/cities/960x720/UTH.jpg', desc: 'ทะเลบัวแดง วัดป่าบ้านตาด' },
  { from: 'BKK', to: 'KKC', name: 'ขอนแก่น', nameEn: 'Khon Kaen', image: 'https://photo.hotellook.com/static/cities/960x720/KKC.jpg', desc: 'พระธาตุขามแก่น บึงแก่นนคร' },
  { from: 'BKK', to: 'NAW', name: 'นราธิวาส', nameEn: 'Narathiwat', image: 'https://photo.hotellook.com/static/cities/960x720/NAW.jpg', desc: 'หาดนราทัศน์ ศาลาแดง มัสยิด' },
  { from: 'BKK', to: 'TST', name: 'ตรัง', nameEn: 'Trang', image: 'https://photo.hotellook.com/static/cities/960x720/TST.jpg', desc: 'เกาะลิบง ถ้ำมรกต หมูย่างเมืองตรัง' },
  { from: 'BKK', to: 'LOE', name: 'เลย', nameEn: 'Loei', image: 'https://photo.hotellook.com/static/cities/960x720/LOE.jpg', desc: 'ภูกระดึง เชียงคาน ผีตาโขน' },
  { from: 'BKK', to: 'HHQ', name: 'หัวหิน', nameEn: 'Hua Hin', image: 'https://photo.hotellook.com/static/cities/960x720/HHQ.jpg', desc: 'ชายทะเลหลวง วังไกลกังวล ตลาดโต้รุ่ง' },
  { from: 'BKK', to: 'UTP', name: 'พัทยา', nameEn: 'Pattaya', image: 'https://photo.hotellook.com/static/cities/960x720/UTP.jpg', desc: 'วอล์คกิ้งสตรีท เกาะล้าน น้อง-ณุช' },
  { from: 'BKK', to: 'MAQ', name: 'แม่สอด', nameEn: 'Mae Sot', image: 'https://photo.hotellook.com/static/cities/960x720/MAQ.jpg', desc: 'สะพานมิตรภาพ ตลาดชายแดน ริมเมย' },
  { from: 'BKK', to: 'PHS', name: 'พิษณุโลก', nameEn: 'Phitsanulok', image: 'https://photo.hotellook.com/static/cities/960x720/PHS.jpg', desc: 'พระพุทธชินราช น้ำตกแก่งโสภา' },
];
...
export default function FlightsPage() {
  ...
  const handleSearch = () => {
    if (!searchText.trim()) {
      window.open(MARKER ? `https://www.aviasales.com/?marker=${MARKER}` : 'https://www.aviasales.com/', '_blank');
      return;
    }
    ...
    <AppLayout>
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="text-center mb-6">
            <h1 className="font-display text-2xl md:text-4xl font-extrabold text-tmain flex items-center justify-center gap-3">
              <Plane size={32} /> เที่ยวบินในประเทศไทย
            </h1>
          ...
          <div className="bg-white rounded-2xl p-4 md:p-6 shadow-xl border border-primary-dark/20">
            <div className="relative mb-3">
              ...
              <input type="text" value={searchText} onChange={(e) => setSearchText(e.target.value)} onKeyDown={handleKeyDown} placeholder="คุณอยากบินไปไหน? เช่น เชียงใหม่, ภูเก็ต, กระบี่..." className="w-full h-14 pl-12 pr-4 rounded-xl border-2 border-primary-dark/30 text-base text-tmain outline-none focus:border-secondary focus:ring-2 focus:ring-secondary/20 placeholder:text-tmuted/60" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select value={passengers} onChange={(e) => setPassengers(parseInt(e.target.value))} className="w-full h-11 px-3 rounded-xl border border-primary-dark/30 text-sm text-tmain outline-none focus:border-primary bg-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h2 className="font-display text-xl font-bold text-tmain mb-2">
          {searchText.trim() ? `ผลการค้นหา "${searchText}"` : 'เส้นทางบินยอดนิยมจากกรุงเทพ'}
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((route) => (
            <a key={route.to} href={buildLink(route.from, route.to)} target="_blank" rel="noopener noreferrer" className="bg-white rounded-2xl overflow-hidden border border-primary-dark/20 hover:border-secondary hover:shadow-lg transition-all group">
              <div className="relative h-44 overflow-hidden">
                <img src={route.image} alt={route.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-3 left-3">
                  <h3 className="font-bold text-white text-lg drop-shadow">{route.name}</h3>
                  <p className="text-white/80 text-xs drop-shadow flex items-center gap-1">{route.from} <ArrowRight size={12} /> {route.to} · {route.nameEn}</p>
                </div>
              </div>
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <span className="inline-flex items-center gap-1 bg-secondary hover:bg-secondary/90 text-tmain font-semibold text-xs px-3 py-1.5 rounded-lg transition">ดูเที่ยวบิน <ExternalLink size={11} /></span>
                </div>
              </div>
            </a>
          ))}
          <a href={MARKER ? `https://www.aviasales.com/?marker=${MARKER}` : 'https://www.aviasales.com/'} target="_blank" rel="noopener noreferrer" className="bg-white rounded-2xl overflow-hidden border-2 border-dashed border-primary-dark/30 hover:border-secondary hover:shadow-lg transition-all group flex flex-col items-center justify-center min-h-[280px]">
            ...
            <h3 className="font-bold text-tmain text-lg">ค้นหาเส้นทางอื่น ๆ</h3>
            <p className="text-sm text-tmuted mt-1">บินไปที่ไหนก็ได้ทั่วไทย</p>
