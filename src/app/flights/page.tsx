...
export default function FlightsPage() {
  ...
  const filtered = searchText.trim()
    ? popularRoutes.filter((r) => r.name.includes(searchText) || r.nameEn.toLowerCase().includes(searchText.toLowerCase()) || r.to.toLowerCase().includes(searchText.toLowerCase()))
    : popularRoutes;
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select value={passengers} onChange={(e) => setPassengers(parseInt(e.target.value))} className="w-full h-11 px-3 rounded-xl border border-primary-dark/30 text-sm text-tmain outline-none focus:border-primary bg-white">
                  <option value={1}>1 คน</option><option value={2}>2 คน</option><option value={3}>3 คน</option><option value={4}>4 คน</option>
                </select>
              </div>
