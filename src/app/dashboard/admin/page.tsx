...
export default function AdminDashboard() {
  ...
  const handleApprove = async (bookingId: string) => {
    ...
    if (booking) {
      await supabase.from('notifications').insert({
        ...
        message: `การจอง "${booking.post?.title}" ได้รับการอนุมัติแล้ว กรุณาชำระเงิน`,
        type: 'booking',
        link: `/booking`,
      });
    }
  ...
  const handleReject = async (bookingId: string) => {
    ...
    if (booking) {
      await supabase.from('notifications').insert({
        ...
        message: `การจอง "${booking.post?.title}" ถูกปฏิเสธ: ${reason}`,
        type: 'booking',
        link: '/booking',
      });
    }
  ...
  const handleOfferAlternative = async (booking: Booking) => {
    ...
    await supabase.from('notifications').insert({
      ...
      message: `Admin เสนอตัวเลือกใหม่สำหรับการจอง "${booking.post?.title}"`,
      type: 'booking',
      link: '/booking',
    });
