...
export default function AdminDashboard() {
  ...
  const getPostTitle = async (bookingId: string): Promise<string> => {
  ...
  const handleApprove = async (bookingId: string) => {
    ...
    const postTitle = await getPostTitle(bookingId);
    ...
    if (booking) {
      await supabase.from('notifications').insert({
        ...
        message: `การจอง "${postTitle}" ได้รับการอนุมัติแล้ว กรุณาชำระเงิน`,
        type: 'booking',
        link: `/booking`,
      });
    }
  ...
  const handleReject = async (bookingId: string) => {
    ...
    if (booking) {
      await supabase.from('notifications').insert({
        ...
        message: `การจอง "${postTitle}" ถูกปฏิเสธ: ${reason}`,
        type: 'booking',
        link: '/booking',
      });
    }
  ...
  const handleOfferAlternative = async (booking: Booking) => {
    ...
    await supabase.from('notifications').insert({
      user_id: booking.customer_id,
      title: 'มีตัวเลือกใหม่สำหรับคุณ',
      message: `Admin เสนอตัวเลือกใหม่สำหรับการจองของคุณ`,
      type: 'booking',
      link: '/booking',
    });
  ...
  return (
    <AppLayout>
      <div className="max-w-6xl mx-auto px-4 lg:px-0 py-6 lg:py-8">
